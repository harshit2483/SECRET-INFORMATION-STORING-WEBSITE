...
`""""""`'''
    ...
    def frame1_encode(self,f):
        ...
        bws_button = Button(f2,text='Select',command=lambda : self.frame2_encode(f2),bg="light green")
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(f2, text='Cancel', command=lambda : Stegno.home(self,f2),bg="light blue")
        back_button.config(font=('courier',18))
        back_button.grid(pady=15)
        back_button.grid()
        f2.grid()
